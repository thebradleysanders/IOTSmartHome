...
class Hue
{
    ...
    public function makeLightArray( $lightid = false )
    {
        $targets = array();
	...
	// Sets the state property
    public function setHueLight($id, $input)
    {
        $data = json_encode( $input );
        $pest = $this->makePest();
        $pest->put( "lights/" .$id. "/state", $data );
    }
    ...
    // Gets an array of currently configured sensors
    public function GetSensors()
    {
        $pest = $this->makePest();
        $result = json_decode( $pest->get( "sensors" ), true );

        return $result;
    }
    
    // Gets an array of currently configured lights
