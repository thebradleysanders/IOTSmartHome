...
class FindMyiPhone {
     ...
     * Example: $fmi->sendMessage("dCsaBcqBOdnNop4wvy2VfIk8+HlQ/DRuqrmiwpsLdLTuiCORQDJ9eHYVQSUzmWV", "title", "message");
     *
     * @param deviceID	ID of the device you want to play a sound
     * @param subject	Title of the message you want displayed on the device
     * @param text	Message you want displayed on the device
     */
	public function sendMessage($deviceID, $subject, $text) {
		$url = "https://fmipmobile.icloud.com/fmipservice/device/".$this->username."/sendMessage";
		$body = json_encode(array("device"=>$deviceID, "subject"=>$subject, "text"=>$text,'userText' => true)); 
		list($headers, $body) = $this->curlPOST($url, $body, $this->username.":".$this->password);
	}
